...
const ShoreTrips = () => {
  ...
  const visibleTrips = useMemo(() => {
    ...
    return shoreTrips
      .filter((trip) => [trip.vesselName, trip.owner, trip.registrationNumber, trip.approval].some((value) => value.toLowerCase().includes(query)))
      .sort((a, b) => sort === "vessel" ? a.vesselName.localeCompare(b.vesselName) : a.scheduledDateTime.localeCompare(b.scheduledDateTime));
  }, [search, sort]);
        ...
        <div className="divide-y divide-slate-100 md:hidden">
          {visibleTrips.map((trip) => (
            <button key={trip.id} type="button" onClick={() => navigate(`/shore/trips/${trip.id}`)} className="block w-full p-5 text-left transition hover:bg-slate-50">
              <div className="flex items-start justify-between gap-3"><strong className="text-sm text-[#14223d]">{trip.vesselName}</strong><Status status={trip.approval} /></div>
              <p className="mt-2 text-xs text-slate-500">{trip.registrationNumber} · {trip.owner}</p><p className="mt-1 text-xs text-slate-400">{formatTripDate(trip.scheduledDateTime)}</p>
            </button>
          ))}
        </div>
...
function TripRow({ trip, onOpen }: { trip: (typeof shoreTrips)[number]; onOpen: () => void }) {
  return <tr className="transition hover:bg-slate-50/70"><td className="px-8 py-5 font-medium text-[#14223d]">{trip.vesselName}</td><td className="px-5 py-5 text-slate-600">{trip.owner}</td><td className="px-5 py-5 text-slate-600">{trip.registrationNumber}</td><td className="px-5 py-5 text-slate-600">{formatTripDate(trip.scheduledDateTime)}</td><td className="px-5 py-5"><Status status={trip.approval} /></td><td className="px-8 py-5 text-right"><button type="button" onClick={onOpen} aria-label={`Review ${trip.vesselName}`} className="rounded-full border border-indigo-200 px-3 py-1 text-xs font-medium text-indigo-700 transition hover:bg-indigo-50">Review</button></td></tr>;
}

function Status({ status }: { status: (typeof shoreTrips)[number]["approval"] }) {
  const color = status === "Approved" ? "text-emerald-600 bg-emerald-50" : status === "Declined" ? "text-red-600 bg-red-50" : "text-amber-600 bg-amber-50";
